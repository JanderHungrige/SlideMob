...
import json
import re
from utils.promts import translation_prompt_llama2_0, translation_prompt_deepseek_0

class LMStudioTranslator(BaseTranslator):
    def __init__(self, model: str, client: any, model_type: str = "unknown", 
    ...
    def translate(self, text: str, target_language: str, style_instructions: str = "") -> str:
        try:
            ...
            if self.model_type == "llama":
                prompt = translation_prompt_llama2_0(text, target_language, style_instructions)
            elif self.model_type == "deepseek":
                prompt = translation_prompt_deepseek_0(text, target_language, style_instructions)
            else:
